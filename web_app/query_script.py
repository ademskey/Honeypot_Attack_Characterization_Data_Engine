...
it clears the honeypot_data.jsonl file before writing to it.
It also prints a progress bar and allows the user to exit the script at any time by pressing Enter.
'''
def collect_honeypot_data(time, debug_input):
    # Variables ############
    total_hits = 0
...
if __name__ == "__main__":
    ...
    total_hits = collect_honeypot_data(time, debug_input)

    if debug_input == "y":
        print(f"\nTotal Hits Collected: {total_hits} over {time} hours")
